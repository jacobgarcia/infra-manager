...
router
  ...
  .post((req, res) => {
    ...
    if (!core || !user || !pass || !streamid || !company || !site || !name || !country || !zone) return res.status(400).json({ success: false, message: 'Malformed request' })
    return Company.findOne({ _id: company }).exec((error, company) => {
      ...
      return Zone.findOne({ _id: zone, company: company._id }).exec((error, zone) => {
        if (error) {
          winston.error(error)
          return res.status(500).json({ error })
        }
        ...
        return Site.findOne({ key: site, company: company._id, zone: zone._id })
          .select('id')
          .exec((error, site) => {
            let issuerkey = null
            let id = null
            ...
            return request.get(
              `${core}/service/trusted/issuer?version=2`,
              {
                auth: {
                  user: user,
                  pass: pass
                    ...
                    return Stream.findOne({ id: JSON.parse(body).id, company: company._id }).exec(
                      (error, stream) => {
                        if (error) {
                          return res.status(500).json({
                            success: false,
                            message: 'Stream base data error',
                            error
                          })
                        }
                        if (stream) {
                        ...
                        return res.status(500).json({
                          success: false,
                          message: 'Issuer ocupated, delete first',
                          error
                        })
                      }
                    )
...
router.route('/stream/token/:id').get((req, res) => {
  const { id } = req.params
  if (!id) return res.status(400).json({ success: false, message: 'Malformed request' })
