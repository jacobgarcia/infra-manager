...
router.route('/polygons/:stateId').get((req, res) => {
  const { stateId } = req.params

  State.findById(stateId).exec((error, state) => {
    if (error) {
      winston.error({ error })
...
router.route('/polygons').get((req, res) => {
  // TODO: Country
  State.aggregate([
    {
      $group: {
        _id: '$country',
        states: { $push: { name: '$name', _id: '$_id', code: '$code' } }
      }
    }
  ]).exec((error, states) => {
    if (error) {
