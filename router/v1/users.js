...
router.route('/users/recognize').post((req, res) => {
  ...
  Company.findOne({ _id: company }).exec((error, company) => {
    if (error) {
      winston.error(error)
      return res.status(500).json({ success: false, message: 'Error at finding users' })
    }
    ...
    .exec((error, thesite) => {
      ...
      return FrmUser.findOne({ pin }).exec((error, user) => {
        ...
        return res.status(200).json({
          success: true,
          message: 'The camera is ready to recognize the user. Now take the picture',
          user
        })
      })
    })
...
router.route('/users/signup').post((req, res) => {
  ...
  return Site.findOne({ key: site }).exec((error, thesite) => {
    if (error) {
      winston.error(error)
      return res.status(500).json({ success: false, message: 'Error finding that site.', error }) // return shit if a server error occurs
    }
    ...
    return FrmUser.findOne({ pin }).exec((error, user) => {
      ...
      return new FrmUser({
        ...
        return res.status(200).json({
          success: true,
          message: "Successfully registered user. Now it's time to take the picture!",
          user
        })
      })
    })
...
router.route('/users/photo').put((req, res) => {
  ...
  if (!photo) return res.status(400).json({ success: 'false', message: 'Image not found' })
  ...
  return Company.findOne({ _id: company }).exec((error, company) => {
    ...
    return base64Img.img(
      ...
      (error, filename) => {
        ...
        FrmUser.findOneAndUpdate({ pin }, { $set: { photo: '/' + filename } }, { new: true }).exec(
          (error, user) => {
            if (error) {
              winston.error(error)
              return res.status(400).json({
